...
while True:
    ...
    if action == 2:
        ...
        if item_q <= 0:
            ...
            if s == 'Yes':
                new_item = create_order_item(food, item_i, item_q)
                order["items"].append(
                    new_item
                )
                ...
                save('food', food)
                print(f"«{food[item_i]['name']}» x{item_q}", 
                    "was successfully added to the order!")
                wait()
        else:
            print(f"Error: This quantity of «{food[item_i]['name']}»",
                "is not available!")
            wait()

    if action == 3:
        print_items( order["items"], "YOUR ORDER", True)
        wait()
