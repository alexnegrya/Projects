...
class WrongStorageMetod(Exception):
  pass
...
class Storage:
  def __init__(self, storage_type=None):
    if storage_type in ['memory', 'file', 'json'] or storage_type == None:
      if storage_type == None or storage_type == 'memory':
        ...
        self._storage = _StorageProxy('file')
      elif storage_type == 'json':
        self._storage = _StorageProxy('json')
    else:
      raise StorageError('unknouwn storage type')
  ...
  def __getattr__(self, name):
    if name in ['load', 'save']:
      ...
      raise WrongStorageMetod('unknown storage metod')