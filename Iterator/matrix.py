class Matrix:
    ...
    def __next__(self):
        if self.__stop_iter: raise StopIteration
        if self.__index_y == len(self.matrix2d) - 1 and self.__index_x == len(
          self.matrix2d[self.__index_y]) - 1:
        ...
        if self.__index_x == 0:
            ...
            self.__index_x = 0
            self.__index_y += 1
            return self.matrix2d[self.__index_y - 1][len(
                self.matrix2d[self.__index_y - 1]) - 1]
        elif self.__index_x > 0 and self.__index_x < len(
          self.matrix2d[self.__index_y]):
